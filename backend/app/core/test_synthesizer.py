...
✅ UPDATED: Enhanced cluster synthesis with correctness checking and assertion enrichment
"""
...
class TestSynthesizer:
    ...
    async def _synthesize_single_cluster(
        ...
        """
        ✅ UPDATED: Synthesize a single cluster with correctness checking and assertion enrichment
        ...
        # ✅ UPDATED: Build enhanced cluster synthesis prompt
        prompt = self._build_cluster_synthesis_prompt_v2(
    ...
    def _build_cluster_synthesis_prompt_v2(
            self,
            cluster_tests: List[str],
            category: str,
            function_name: str,
        ...
        """
        ✅ NEW: Enhanced cluster synthesis prompt with correctness checks and assertion enrichment

        Changes:
        1. Explicit correctness validation step
        2. Request for missing assertions/scenarios
        3. Simplified instructions for lightweight LLMs
        """

        # Format all tests in the cluster
        tests_formatted = []
        for idx, test in enumerate(cluster_tests, 1):
            tests_formatted.append(f"""Test {idx}:
```python
{test}
```""")
        ...
        prompt = f"""You are reviewing and improving test code.

## Function to Test:
```python
{function_code}

## Existing Tests (Category: {category}):
{all_tests_text}

## Your Task - Do these 3 steps:

**Step 1: Check Correctness**
- Review each test above
- Fix any bugs or incorrect assertions
- Remove tests that don't make sense for this function

**Step 2: Find Missing Coverage**
- What scenarios are NOT tested yet?
- What assertions could be added?
- What edge cases are missing?

**Step 3: Create ONE Complete Test**
Write a single test function that:
- Fixes all bugs from the tests above
- Combines all valid assertions
- Adds 2-3 NEW assertions for missing scenarios
- Uses clear variable names
- Has a descriptive docstring

## Rules:
- Test name: `test_{function_name}_<what_it_tests>`
- Include docstring
- NO imports (added later)
- Output ONLY the test function

Generate the improved test now:
"""
